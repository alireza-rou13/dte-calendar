...
export default function Calendar({ ...props }) {
  ...
  const jalaiDate = moment(new Date(curentActiveMonth))
    .locale("fa")
    .format("YYYY/MM/DD");
  ...
  const calculateCalendarDays = (activeDate) => {
    ...
    const jalaiMonthStartDate = moment(new Date(activeDate))
      .locale("fa")
      .format("YYYY/MM/01");
    const hlpDate = new Date(
      moment.from(jalaiMonthStartDate, "fa", "YYYY/MM/DD").format("YYYY-MM-DD")
    );
    const calendarStartDate = hlpDate;

    if (hlpDate.getDay() !== 6) {
      calendarStartDate.setDate(
        calendarStartDate.getDate() - (hlpDate.getDay() + 1)
      );
    }
    ...
    setHlpMonthStartDayDate(
      new Date(
        moment
          .from(jalaiMonthStartDate, "fa", "YYYY/MM/DD")
          .format("YYYY-MM-DD")
      )
    );
    const jalaiMonthEndDate = moment(new Date(activeDate))
      .locale("fa")
      .add(1, "jMonth")
      .format("YYYY/MM/01");
    setHlpMonthEndDayDate(
      new Date(
        moment.from(jalaiMonthEndDate, "fa", "YYYY/MM/DD").format("YYYY-MM-DD")
      )
    );
  };

  const showNextMonth = () => {
    const jalaiNextMonth = moment(new Date(curentActiveMonth)).add(
      1,
      "jMonth"
    )._d;
  ...
  const showPreviousMonth = () => {
    const jalaiPreviousMonth = moment(new Date(curentActiveMonth)).subtract(
      1,
      "jMonth"
    )._d;
  ...
  const dayCellDidMount = (hlpMonthStartDayDate, hlpMonthEndDayDate) => {
    for (let i = 0; i < calendar.length; i++) {
      calendar[i].className = calendar[i].className.replace(
        " fc-day-other",
        ""
      );
      if (
        new Date(calendar[i].getAttribute("data-date")).getTime() >=
          new Date(hlpMonthEndDayDate).getTime() ||
        new Date(calendar[i].getAttribute("data-date")).getTime() <
          new Date(hlpMonthStartDayDate).getTime()
      ) {
  ...
  const handle_event = (info) => {
    ...
    var ap_icon =
      info.extendedProps.approved !== "1"
        ? `  <i class="bi bi-exclamation-triangle-fill"></i>  `
        : `  <i class="bi bi-check-square-fill"></i>  `;
    var ap_class = info.extendedProps.approved
    ...
    const _otherfield = `<div class="d-inline"><p class="d-inline fw-bold">استاد/ استادان :</p> ${info.extendedProps.field_professors} <br />
    <p class="d-inline fw-bold">تعداد مدعوین : </p> ${info.extendedProps.field_guests} <br />
    <p class="d-inline fw-bold"> سطح جلسه </p> ${info.extendedProps.field_level} </div>`;
    setbody(_alert + _approve + _otherfield + info.extendedProps.description);
  };
  const check_approved = (code) => {
    switch (code) {
      case "0":
        return "در دست بررسی است";
        break;
      case "1":
      ...
      {isLoadComplete && (
        <>
          <div className="claendar-toolbar">
            ...
            {/*    نمایش هفته*/}
            {/*</div>*/}
            <div className="toolbar-arrow-right" onClick={showPreviousMonth}>
              <RightIcon />
              ماه قبل
            </div>
            <h1>
            ...
            <div className="left-toolbar">
              <div className="toolbar-today" onClick={showToday}>
                امروز
              </div>
              <div className="toolbar-arrow-left" onClick={showNextMonth}>
                ماه بعد
                <LeftIcon />
              </div>
            </div>
          ...
          <FullCalendar
            ...
            visibleRange={range}
            dayCellDidMount={() =>
              dayCellDidMount(hlpMonthStartDayDate, hlpMonthEndDayDate)
            }
            headerToolbar={false}
            eventClick={(info) => {
